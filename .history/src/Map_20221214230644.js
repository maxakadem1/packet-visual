...
export default function Map({
  ...
  useEffect(() => {
    if (packetData) {
      let newPacketData = {
        coordinates: [packetData.destLong, packetData.destLat],
      };
      setPacket([...packets, newPacketData]);

      let newPacketLinksData = {
        to: [packetData.destLong, packetData.destLat],
        from: [packetData.sourceLong, packetData.sourceLat],
      };
  ...
  useEffect(() => {
    packetLinksRef.current = packetLinks;
  });
  useEffect(() => {
    setInterval(() => {
      clearPacket();
    ...
    >
      ...
      {packetData && (
        <div className="rawData">
          <h4 style={{ transform: "translate(2vw, 40vh)" }}>
            Incoming packet!
          </h4>
          <ol style={{ transform: "translate(2vw, 40vh)" }}>
            ...
            <li>
              Dest Location: LAT {packetData.destLat} , LONG{" "}
              {packetData.destLong}
            </li>
          </ol>
        </div>
